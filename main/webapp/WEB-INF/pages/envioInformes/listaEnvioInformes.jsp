...
<div id="main-content">
<br><br>
<center><h3>Listado para el envío de informes a Padrinos Individuales</h3></center>
<br><br>
<form method="post" id="form" name="form" action="<c:url value="/envioInformes/listaInformes.do"/>">
<CENTER>

<!-- filtros -->
<fieldset style="font-size: 12px; margin-left: 47px; margin-right: 47px; width:1100px;">
<table id="table-informe" align="center" style="margin-left: 100px; margin-right: 60px; border-style:hidden;" width="1100px">
		<tr align="center">
...
<display:table  export="true" class="displayTags_wrapper" style="font-size: 12px; margin-left: 50px; margin-right: 50px;width:1200px; background-color: #C0C2FF;" uid="informe" name="informes" pagesize='50' defaultsort="1" defaultorder="ascending" requestURI="">				
	    ...
	    <display:column  headerClass="sortable" title="Zona" style="width: 8%" >
	    <c:choose>	    
	    	<c:when test="${informe.descripcionPadrino=='Rosenkrantz, Carlos'}">
	    		<c:choose>	    
	    			<c:when test="${informe.beca.ciclo.nombre=='2014'}" >
	    				<c:choose>
	    					<c:when test="${informe.nombreInforme=='Informe de Seguimiento Nº1'}" >	
	    						<c:choose>
	    							<c:when test="${informe.beca.zona.nombre=='Villa la Angostura'}">
		    							San Martín de los Andes
		    						</c:when>
		    						<c:otherwise>
										${informe.beca.zona.nombre}
									</c:otherwise>	    
								</c:choose>
							</c:when>
							<c:when test="${informe.nombreInforme=='Informe de Cesación'}" >	
	    						<c:choose>
	    							<c:when test="${informe.beca.zona.nombre=='Villa la Angostura'}">
		    							San Martín de los Andes
		    						</c:when>
    ...
    <display:setProperty name="export.excel.filename" value="informesPadrinosIndividuales.xls" />
</display:table>
</CENTER>
</form>
</div>
