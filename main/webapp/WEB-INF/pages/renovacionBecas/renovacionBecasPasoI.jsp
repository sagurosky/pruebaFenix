...
<title>Renovaci&oacute;n Becas Paso 1</title>
...
$(document).ready(function() {
	...
	$('#boton_todos').click( function() {
		...
		oTable2.fnReloadAjax( "../renovacionBecas/buscarAlumnos.do?paso=" + 1);
		document.getElementById("botonSelecTodos").disabled = true;
	})
	...
	$('#asignacionForm').submit( function() {
		...
		if(alumnosSeleccionados == "" || becaSeleccionada == ""){
	        jAlert('warning', 'Por favor seleccione 1 beca y al menos 1 alumno.', 'Atención!');
			return false;
		}
		return true;
	} );
	...
	oTable1 = $('#tabla1').dataTable({
	 ...
	 oTable2 = $('#tabla2').dataTable({
		...
		"sPaginationType": "full_numbers",
		"iDisplayLength": 25,
		"sAjaxSource" : "../renovacionBecas/buscarAlumnos.do?paso=" + 1,
		"aoColumns" : [
					/* idCandidato */{
						"bSortable" : false,
						"bSearchable" : false,
	 ...
	 $("#tabla1 tbody").delegate('#beca_row', 'click', function(event) {
			...
			if(aData != null){
				oTable2.fnReloadAjax( "../renovacionBecas/buscarAlumnos.do?becaId=" + aData[0] + "&paso=" + 1);
				document.getElementById("botonSelecTodos").disabled = false;
			}
...
<body>
<div id="main-content">
<form id="asignacionForm" class="asignacionForm" action="<c:url value='/renovacionBecas/renovacionBecas.do?paso=1' />" method="post">
<br>
<CENTER>
...
<h3 align="center">Renovaci&oacute;n Becas Paso 1</h3>
</div>
...
<tr>
	<td>
		...
		<input align="middle" id="botonSelecTodos" type="button" value="Seleccionar todos" class="ui-state-default ui-corner-all" disabled="disabled"></input>	
