...
function enviarForm(action, id){
	if(action=='modificar'){
		document.forms['provinciaForm'].action = "../provincia/modificarView.do";
	}else if(action=='eliminar'){
		document.forms['provinciaForm'].action = "../provincia/eliminarProvincia.do";
	}
	document.forms['provinciaForm'].idProvincia.value = id;
	document.forms['provinciaForm'].submit();
}
</script>
</head>
...
<h3 align="center">Listado de Provincias</h3>
</div>
<!-- Fin titulo -->
<div class="ui-state-default ui-corner-all" align="center">
	...
	<tr>
		<td>
			<input type="submit" value="Alta" class="ui-state-default ui-corner-all" onclick="window.location.href='../provincia/registrarView.do'" />
		</td>
	</tr>
	</table>
</div>
<table align="center"  class="dataTables_wrapper" id="laTabla" width="100%" cellspacing="1">
		...
		<tbody align="center">
			<c:forEach items="${provincias}" var="provincia">
				<tr>
					<td align="center" valign="top" style="font-size: 14px" class="col0">${provincia.descripcion}</td>
					<td align="center" valign="top" style="font-size: 14px" class="col12">${provincia.pais}</td>
					<td align="left" valign="middle" style="font-size: 14px" class="col1">
					<c:forEach var="localidad" items="${provincia.localidades}">
						${localidad.nombre}
					</c:forEach>
					</td>
				...
				<td>
					<div  style= width:0.5cm class="ui-state-default ui-corner-all" > 
						<a  onclick="enviarForm('modificar','${provincia.id}')"> 
				...
				<td>
					<div  style= width:0.5cm class="ui-state-default ui-corner-all" > 
						<a  onclick="enviarForm('eliminar','${provincia.id}')" > 
...
<form:form name="provinciaForm" method="post" commandName="provincia">
