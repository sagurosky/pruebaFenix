...
<title>Envio Informes Seguimiento Nº 1</title>
...
function enviarForm(action, id){
	...
	if(action=='impresion'){
		document.forms['informeForm'].action = "../informeIS1/exportarInformeIS1.do";
	}
...
<form id="informeForm" class="informeForm" action="<c:url value='/informeIS1/verInformeIS1CorrectoraView.do' />" method="post">
	<center>
		<h1>
			Listado Informes Seguimiento Nº 1 pendientes de Envio 
		</h1>
	</center>
	<br/>
	<CENTER>
								...
								<tr>
									<td>
										<div  style= width:0.5cm class="ui-state-default ui-corner-all" > 
											<a  onclick="enviarForm('impresion','${i.id}')">
											<span class="ui-icon ui-icon-print" title="Informe IS1"> 
											</span></a>				
										</div>
									</td>
								</tr>
