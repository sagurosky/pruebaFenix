...
$(document).ready(() => {
	...
	$("#evaluacionRenovacion").change(function(){
		...
		if(renovacion==5){
			...
			$('#motivoExcepcion').removeAttr('readOnly');			
			//$('#observacionesExcepcion').removeAttr('readOnly');
			...
			$("#ocultarPendiente3").hide(1000);
			$("#ocultarPendiente4").show(1000);

			$("#ocultarPendiente5").hide(1000);
		...
		if(renovacion==2){
			...
			$('#motivoExcepcion').attr('readOnly','readOnly');
			//$('#observacionesExcepcion').attr('readOnly','readOnly');
			...
			$("#ocultarPendiente3").show(1000);
			$("#ocultarPendiente4").hide(1000);
...
<script type="text/javascript">
 ...
 <script>
	function guardarAprobar(valor, valorPerfil, boletin){
		...
		if(document.forms['altaEntrevistaRenovacion'].evaluacionRenovacionMergeada.value==5){
		...
		if(document.forms['altaEntrevistaRenovacion'].evaluacionRenovacionMergeada.value==1){
			if(document.forms['altaEntrevistaRenovacion'].situacionEscolarMergeada.value==""){
				jAlert('warning', 'Falta seleccionar Situación Escolar', 'Atención!');  
				return;	
			}
			
			if(document.forms['altaEntrevistaRenovacion'].observacionesExcepcion.value==""){
				jAlert('warning', 'Falta Detallar información sobre la situación de renovación del estudiantes', 'Atención!');  
				return;	
			}
			...
			if(document.forms['altaEntrevistaRenovacion'].observacionesExcepcion.value==""){
				jAlert('warning', 'Falta Detallar información sobre la situación de renovación del estudiante 2', 'Atención!');  
				return;	
			}

			
			
		}else if(document.forms['altaEntrevistaRenovacion'].evaluacionRenovacionMergeada.value==20){
...
<sql:query var="users">
SELECT * FROM boletin_nuevo where ciclo=2022 and idBecado=${alumno.id}
</sql:query>

<c:forEach var="name" items="${users.rows}">
<c:set var="conBoletin" value="SI"/>
	...
	<div class="col-lg-2">
		...
		<div class="position-fixed">
			...
			<div class="form-group">			
				...
				<div class="form-group">				
					<c:if test="${conBoletin == 'SI'}">
						<c:if test="${tipoPerfil == 'rr'}">
							<input type="button" class="btn btn-secondary " value="Finalizar" class="ui-state-default ui-corner-all" onclick="guardarAprobar('f','${tipoPerfil}','${boletinFinalizado}');" />
						</c:if>
...
<div class="form-group form-inline">
	<label class="text-danger">Situación escolar &nbsp;&nbsp;</label>						
...
<div id="ocultarPendiente3" name="ocultarPendiente3">
<div class="form-group form-inline">
...
<div id="ocultarPendiente5" name="ocultarPendiente5">
<div class="form-group form-inline">
...
<div id="ocultarPendiente4" name="ocultarPendiente4">
...
<div class="form-group form-inline">
	...
	placeholder="Brindar información cualitativa sobre el cierre del ciclo lectivo anterior. En caso de que adeude materias detallar ¿cuáles son? ¿qué dificultades tiene con respecto a la materia? ¿hubo mesas de examen?, ¿cómo le fue en las últimas mesas?. Esta información se envía al padrino">${entrevista.observacionesExcepcion}</textarea>
...
<div class="form-group form-inline">
	<label>Acta de Compromiso firmada por EP y RA &nbsp;&nbsp;&nbsp;</label>
	<select class="form-control" name="actaCompromiso" id="actaCompromiso" style='width:auto'>			
			<c:choose>
  				<c:when test="${entrevista.actaCompromiso == 'Sí'}">
     				<option value="0">Seleccione una opci&oacute;n:</option>
					<option value="Sí" selected="selected">Sí</option>
					<option Value="No">No</option>
				</c:when>
				<c:when test="${entrevista.actaCompromiso == 'No'}">
     				<option value="0">Seleccione una opci&oacute;n:</option>
