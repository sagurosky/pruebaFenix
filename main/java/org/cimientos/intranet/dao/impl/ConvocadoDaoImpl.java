...
public class ConvocadoDaoImpl extends SpringHibernateDao<ConvocadoDTO> implements
	...
	public List<ConvocadoDTO> listaConvocados(int firstResult, int maxResults,
										      ...
										      MotivoEnvioTS motivoEnvioTS, RecomendacionesTS recomendacionesTS, AnioEscolar anio,String eae) {
		...
		boolean tieneFiltro = createCriteriaFiltros(estadoAlumnos, cicloPrograma, 
							 ...
							 ts, motivoEnvioTS, recomendacionesTS, anio, criteria,eae);
		
		//System.out.println("First result: "+ firstResult);
		//System.out.println("Max result: "+ maxResults);
		...
		for (PerfilAlumno alumno  : list) {
			...
			//System.out.print("id alumno: "+idAlumno + " - ");
			if(sel != null){				
				...
				convocadoDTO.setEae(alumno.getEae());
				...
				//System.out.println(convocadoDTO.getNombreAlumno());
	...
	private Map<Long,EntrevistaSeleccion> entrevistaSeleccionPorAlumno(List list,boolean tieneFiltro){
		...
		if(tieneFiltro){
			...
			*/
			// cambié el query porque la fecha de carga de la entrevista puede ser distinta a la fecha de la entrevista 12-03-2015
			String sql = " select distinct ent.id_perfil_alumno, ent.estado_entrevista_alumno ," +
		 			 " ent.nombre_ciclo, con.nombre, ent.motivo_enviots " + 
					 " from entrevista_seleccion ent ,alumno_seleccion alum,convocatoria_seleccion con "+ 
					 " where ent.id_perfil_alumno in(:idAlumno) and ent.convocatoria = con.id_convocatoria ";
			
			
			sql="select distinct perfil_alumno.id, entrevista_seleccion.nombre_ciclo, zona_cimientos.nombre, escuela.nombre, "+
					" anio_escolar.anio_escolar, perfil_alumno.eae,	persona.apellido, persona.dni, perfil_alumno.estado_alumno, "+
					" entrevista_seleccion.estado_entrevista, convocatoria_seleccion.nombre " +
					" from entrevista_seleccion, zona_cimientos,escuela,anio_escolar,perfil_alumno,persona,convocatoria_seleccion "+
					" where entrevista_seleccion.id_perfil_alumno=perfil_alumno.id 	and perfil_alumno.datos_personales=persona.id "+
					" and perfil_alumno.escuela=escuela.id and escuela.zona_cimientos=zona_cimientos.id " +
					" and convocatoria_seleccion.id_convocatoria=entrevista_seleccion.convocatoria " +
					" and perfil_alumno.anio_escolar=anio_escolar.id_anio " +
					" and (perfil_alumno.estado_alumno=1 or perfil_alumno.estado_alumno=3 or perfil_alumno.estado_alumno=11 " +
					" or perfil_alumno.estado_alumno=12 or perfil_alumno.estado_alumno=17) and anio_escolar.id_anio<15 "+
					" and perfil_alumno.id in(:idAlumno)" +
					" order by entrevista_seleccion.nombre_ciclo desc, persona.apellido ";
			...
			for (Object[] objects : list1) {
				...
				*/
				if (objects[0]!=null) {
					sel = new EntrevistaSeleccion();
					Integer estadoAlumno = (Integer) objects[8];
					/*
					if (objects[4]!=null) {
						Integer motivoTs = (Integer) objects[4];
						sel.setMotivoEnvioTS(MotivoEnvioTS.getMotivoEnvioTS(motivoTs+1));
					...
					*/
					sel.setEstadoEntrevistaAlumno(EstadoAlumno.getEstados(estadoAlumno));
					sel.setNombreCiclo((String) objects[1]);
					sel.setConvocatoria(new ConvocatoriaSeleccion());
					sel.getConvocatoria().setNombre((String)objects[10]);
					map.put(((BigInteger)objects[0]).longValue(), sel);
				}
			...
			*/
			...
			String sql = " select distinct ent.id_perfil_alumno, ent.estado_entrevista_alumno ," +
						 " ent.nombre_ciclo, con.nombre, ent.motivo_enviots " + 
						 " from entrevista_seleccion ent ,alumno_seleccion alum,convocatoria_seleccion con "+ 
						 " where ent.convocatoria = con.id_convocatoria";
	...
	public Integer cantidadConvocados(List<EstadoAlumno> estadoAlumnos,CicloPrograma cicloPrograma, ZonaCimientos zona, 
		...
		createCriteriaFiltros(estadoAlumnos, cicloPrograma, zona, escuela,
				...
				recomendacionesTS, anio, criteria,eae);
		criteria.setProjection(Projections.rowCount());
		List list = criteria.list();
		//System.out.println("List cantidad: "+ list.size());
		//System.out.println("Cantidad: "+(Integer)list.get(0));
	...
	private boolean  createCriteriaFiltros(List<EstadoAlumno> estadoAlumnos,
			...
			AnioEscolar anio, Criteria criteria, String eae) {
		...
		if(cicloPrograma != null  && convocatoria != null ){
			...
			if(cicloPrograma != null ){
				...
				/*
				String sql="select distinct perfil_alumno.id, entrevista_seleccion.nombre_ciclo, zona_cimientos.nombre, escuela.nombre, "+
						" anio_escolar.anio_escolar, perfil_alumno.eae,	persona.apellido, persona.dni, perfil_alumno.estado_alumno, "+
						" entrevista_seleccion.estado_entrevista, convocatoria_seleccion.nombre " +
						" from entrevista_seleccion, zona_cimientos,escuela,anio_escolar,perfil_alumno,persona,convocatoria_seleccion "+
						" where entrevista_seleccion.id_perfil_alumno=perfil_alumno.id 	and perfil_alumno.datos_personales=persona.id "+
						...
						" and (perfil_alumno.estado_alumno=1 or perfil_alumno.estado_alumno=3 or perfil_alumno.estado_alumno=11 " +
						" or perfil_alumno.estado_alumno=12 or perfil_alumno.estado_alumno=17) and anio_escolar.id_anio<15 " +
						" and entrevista_seleccion.nombre_ciclo ="+ciclo;
				//System.out.println(sql);
				
				//criteria.add(Restrictions.eq("entrevistaSeleccion.nombreCiclo",ciclo));
				 */ 
