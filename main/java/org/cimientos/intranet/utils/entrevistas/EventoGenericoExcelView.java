...
public class EventoGenericoExcelView extends AbstractExcelView{
	...
	protected void buildExcelDocument(Map<String,Object> model, HSSFWorkbook workbook,
		...
		eventoGenerico=obtenerEvento(idevento,ciclo);
	...
	public static synchronized ArrayList<EventoGenericoDTO> obtenerEvento(Long idevento, String ciclo) {
        ...
        try {
        	...
        	if(ciclo.equals("0")){
        		...
        		if (idevento==null){
        			call = "{CALL SP_EventoGenericoTodos()}";
        		}else{
    				if (idevento>10000){
    						call = "{CALL SP_EventoGenerico1(?)}";
    				}else{
    						call = "{CALL SP_EventoGenerico1T(?)}";
    				}
        		}
        		...
        		if (idevento==null){
        			call = "{CALL SP_EventoGenericoCiclo(?,?)}";
                ...
                if (idevento!=null){
                	...
                	if(!ciclo.equals("0")){
	                	if (idevento>10000){
							call = "{CALL SP_EventoGenerico1Ciclo(?,?,?)}";
							cl = cn.prepareCall(call);
							cl.setLong(1, idevento);
		                	cl.setString(2, ciclo+"-01-01");
	                		cl.setString(3, ciclo+"-12-31");
	                		
	                	
	                	}else{
							call = "{CALL SP_EventoGenerico1TCiclo(?,?,?)}";
							cl = cn.prepareCall(call);
							cl.setLong(1, idevento);
		                	cl.setString(2, ciclo+"-01-01");
	                		cl.setString(3, ciclo+"-12-31");
	                ...
	                if(ciclo.equals("0")){
		                	...
		                	if (idevento==null){
		                		cl = cn.prepareCall(call);
		                		cl.setString(1, ciclo+"-01-01");
			                	cl.setString(2, ciclo+"-12-31");
		                	}else{
		                		if (idevento>10000){
		    						call = "{CALL SP_EventoGenerico1Ciclo(?,?,?)}";
			                	}else{
	...
	private void crearFila(EventoGenericoDTO e, ExportacionEspecialHelper exportacionHelper, HSSFRow row) {
		...
		row.createCell(8).setCellValue(e.getOtroContenido());
		row.createCell(9).setCellValue(e.getEvaluacion());
		row.createCell(10).setCellValue(e.getObservaciones());
		row.createCell(11).setCellValue(e.getConvocados());
		row.createCell(12).setCellValue(e.getAsistio());
		row.createCell(13).setCellValue(e.getResponsable1());
		row.createCell(14).setCellValue(e.getVinculo1());
		row.createCell(15).setCellValue(e.getResponsable2());
		row.createCell(16).setCellValue(e.getVinculo2());
