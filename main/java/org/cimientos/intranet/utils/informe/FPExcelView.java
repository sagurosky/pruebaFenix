...
public class FPExcelView extends AbstractExcelView{
	...
	private void crearFila(ReporteInformeFPDTO fp, HSSFRow row) {		
		...
		*/
		...
		//2023
		row.createCell(84).setCellValue(fp.getVosMismo());
		
		
		row.createCell(87).setCellValue(fp.getObservacionesNoIncorporacion());
		row.createCell(88).setCellValue(fp.getPropositoAnual());
		//row.createCell(89).setCellValue(fp.getMatriculaTotal());
		//row.createCell(90).setCellValue(fp.getOrientacion());
		//row.createCell(91).setCellValue(fp.getMatriculaTotal());
		//row.createCell(92).setCellValue(fp.getDescripcionEscuela());
		//row.createCell(93).setCellValue(fp.getAnioIncorporacion());
		
		String situacionEscolar2="";
		
		
		if(fp.getSituacionEscolar()!=null){
		
			if(fp.getSituacionEscolar().equals("0")){
				situacionEscolar2="Promovido sin adeudar materias";
			}else 
			if(fp.getSituacionEscolar().equals("1")){
				situacionEscolar2="Pendiente";
			}else 
			if(fp.getSituacionEscolar().equals("2")){
				situacionEscolar2="Repitio";
			}else
			if(fp.getSituacionEscolar().equals("3")){
				situacionEscolar2="Egresó";
			}else 
			if(fp.getSituacionEscolar().equals("4")){
				situacionEscolar2="Promovido con materias a rendir";
			}else
				situacionEscolar2=fp.getSituacionEscolar();
			
			fp.setSituacionEscolar(situacionEscolar2);
			row.createCell(85).setCellValue(situacionEscolar2);
		}
		
		if(fp.getIncorporacion()!=null){
		String incorporacion="";
		
		if(fp.getIncorporacion().equals("0")){
			incorporacion="Incorpora";
		}else 
		if(fp.getIncorporacion().equals("1")){
			incorporacion="No Incorpora";
		}else 
		if(fp.getIncorporacion().equals("2")){
			incorporacion="Incorporación pendiente";
		}else
		
			incorporacion=fp.getIncorporacion();
		
		fp.setIncorporacion(incorporacion);
		row.createCell(86).setCellValue(incorporacion);
		}
