...
//			informe.setSituacion(informe.getEr().getSituacionEscolar());
			...
			informe.setHsTrabajarAño(informe.getEr().getHsTrabajarAño());
			informe.setTiempoLibre(informe.getTiempoLibre());
			...
			informe.setPropositoAnioComienza((entrevistaRenovacion.getPropositoAnioComienza()));
	...
	{		
		...
		//copio algunos campos del IS2 generado en el IC si es que el IS2 aún no fue enviado
		if(informeIS2 != null){
	...
	public void generarInformeFP(PerfilPadrino padrino, Beca beca, PerfilAlumno alumno, EstadoInforme estado, boolean porRevision,
			...
			if(es != null){
				...
				ficha.setMateriasMasCuesta(agregarMateriasC(es));
				ficha.setObservaciones(alumno.getEscuela().getObservaciones());				
				ficha.setEscuelaEspacioApoyo(agregarApoyoEscuela(alumno));
				ficha.setCuandoTermine(es.getCuandoTermine());
				ficha.setTiempoLibre(es.getTiempoLibre());
			}
			else
				ficha.setBoletinSeleccions(null);
			ficha.setBeca(beca);
	...
	private String agregarMateriasC(EntrevistaSeleccion es) {
		String resul = "";
		resul = validarMateria(MateriaBoletinSeleccion.LENGUA.getValor(), resul, es.getMateriaLenguaC());
		resul = validarMateria(MateriaBoletinSeleccion.MATEMATICA.getValor(), resul, es.getMateriaMatemC());
		resul = validarMateria(MateriaBoletinSeleccion.CIENCIAS_SOCIALES.getValor(), resul, es.getMateriaCiencSocialesC());
		resul = validarMateria(MateriaBoletinSeleccion.INGLES_LENGUA_EXTRANJERA.getValor(), resul, es.getMateriaInglesLengExtC());
		resul = validarMateria(MateriaBoletinSeleccion.EDUCACION_FISICA.getValor(), resul, es.getMateriaEduFisicaC());
		resul = validarMateria(MateriaBoletinSeleccion.EDUCACION_PLASTICA_ARTISTICA.getValor(), resul, es.getMateriaEduArtC());
		resul +=(!resul.equals(""))?".":"";
		return resul;
		
	}
	private String agregarApoyoEscuela(PerfilAlumno pa) {
		String resul = "";
		Connection cn = null;
        CallableStatement cl = null;
        ResultSet rs = null;
        String call=null;          	        	
        call = "{CALL SP_TraerApoyoEscuela(?)}";
        cn = Conexion.getConexion();
        try {
			cl = cn.prepareCall(call);
			cl.setLong(1, pa.getEscuela().getId());			             
            rs = cl.executeQuery();
            while (rs.next()) {	                    
            	resul=rs.getString("element")+" - ";	            
            }
	...
	public void generarComunicadoNoRenovacion(PerfilPadrino padrino, Beca beca, EntrevistaIndividual entrevista,PerfilAlumno alumno){
		...
		if(fp != null && fp.getFpPorReasignacion() && !fp.getEstado().equals(EstadoInforme.ENVIADO)){ //se inactiva la FP y no se genera el Comunicado
			...
			// saqué para que se hagan los comunicados a todos
			...
			List<EntrevistaIndividual> entrevistasPagosSuspendidos, List<EntrevistaIndividual> entrevistasCicloActual) 
	{
		...
		informe.setCicloActual(srvCiclo.obtenerCicloActual());
		//2018
		informe.setHsTrabajarAño(entrevista.getHsTrabajarAño());
		informe.setSarpepe(entrevista.getSarpepe());
		informe.setQtam(entrevista.getQtam());
		informe.setOsme(entrevista.getOsme());
		informe.setAlo(entrevista.getAlo());
		informe.setEstadoIS3("0");
